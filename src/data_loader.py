...
def load_processed_kpis(path: str = "../data/processed/compustat_kpis.csv") -> DataFrame:
    df = pd.read_csv(path)
...
def load_processed_macro(path: str = "../data/processed/macro_data.csv") -> pd.DataFrame:
    """
    Load processed macro data from CSV.
    No additional cleaning apart from reading the CSV.
    """
...
def load_final_panel(path: str | None = None) -> DataFrame:
    """
    Load the final balanced firm-year panel with KPIs, macro variables and target y.
    By default, loads data/final/panel_balanced_with_deltas.csv relative to the project root.
    """
    if path is None:
        # project_root = parent folder of src/
        project_root = os.path.dirname(os.path.dirname(__file__))
        path = os.path.join(project_root, "data", "final", "panel_balanced_with_deltas.csv")
